...
import { toLatinDigits, toLocaleDigits } from "@/presentation/lib/format";
...
export function KioskScreen({ gymName }: { gymName: string }) {
  ...
  const submit = useCallback(
    async (memberCode: string, method: "code" | "qr") => {
      ...
      try {
        // The keypad renders localized digits; the API only ever sees ASCII.
        const outcome = await checkIn({
          memberCode: toLatinDigits(memberCode),
          method,
          deviceToken,
        }).unwrap();
        dispatch(kioskCheckInSucceeded(outcome));
      } catch (caught) {
        dispatch(kioskCheckInFailed(apiErrorMessage(caught, t("failed"))));
      }
    },
    [checkIn, deviceToken, dispatch, t],
  );
  ...
  return (
    <main className="relative flex min-h-dvh flex-col items-center justify-center overflow-hidden bg-background px-6 py-10">
      ...
      {/* Oversized and always visible, so a member can pick their own language. */}
      ...
      <button
        ...
        className="absolute top-8 end-8 text-xs text-muted-foreground/40 transition-colors hover:text-muted-foreground"
      >
        {t("unpair")}
      </button>
      ...
      {stage === "result" ? (
        ...
        <div className="relative z-10 w-full max-w-md text-center">
          ...
          <h1 className="text-xl font-semibold tracking-tight">{t("welcome", { gym: gymName })}</h1>
          <p className="mt-2 text-base text-muted-foreground">{t("enterCode")}</p>

          <div
            className="mx-auto mt-9 flex h-20 items-center justify-center gap-2.5"
            aria-live="polite"
            aria-label={t("codeEntered", { code: code ? toLocaleDigits(code, { locale }) : t("noCode") })}
          >
            {Array.from({ length: 6 }).map((_, index) => (
              ...
              >
                {code[index] ? toLocaleDigits(code[index], { locale }) : "·"}
              </span>
            ))}
          </div>
          ...
          <div className="mx-auto mt-8 grid w-80 grid-cols-3 gap-3">
            {KEYPAD.map((digit) => (
              <KeypadButton key={digit} onClick={() => dispatch(digitPressed(digit))}>
                {/* Shows ۰۱۲۳ in Dari and Pashto; stores the canonical value. */}
                {toLocaleDigits(digit, { locale })}
              </KeypadButton>
            ))}
            ...
            <KeypadButton
              ...
              aria-label={scannerActive ? t("stopScanning") : t("scanQr")}
            >
              {scannerActive ? <CameraOff className="size-6" /> : <Camera className="size-6" />}
            </KeypadButton>

            <KeypadButton onClick={() => dispatch(digitPressed("0"))}>
              {toLocaleDigits("0", { locale })}
            </KeypadButton>

            <KeypadButton muted onClick={() => dispatch(digitRemoved())} aria-label={t("delete")}>
          ...
          {scannerActive ? (
            <QrScanner
              ...
              onUnsupported={() => {
                ...
                dispatch(kioskCheckInFailed(t("cannotScan")));
              }}
            />
          ) : null}
        </div>
  ...
  if (error) {
    return (
      <div className="relative z-10 max-w-lg text-center">
        ...
        <h1 className="mt-7 text-2xl font-semibold tracking-tight">{t("seeFrontDesk")}</h1>
  ...
  return (
    <div className="relative z-10 max-w-2xl animate-[var(--animate-check-in)] text-center">
      ...
      <h1 className="mt-8 text-4xl font-semibold tracking-tight text-balance">
        {result.outcome === "already_inside" ? t("alreadyIn") : t("welcomeBack")}
        <br />
        <span className="text-primary">{firstName}</span>
      </h1>

      <p className="mt-5 text-lg text-muted-foreground">
        {t("haveAGreatSession", { gym: gymName })}
      </p>
    ...
    <main className="flex min-h-dvh items-center justify-center bg-background px-6">
      ...
      >
        ...
        <div className="space-y-1.5">
          <h1 className="text-lg font-semibold tracking-tight">{t("pairTitle")}</h1>
